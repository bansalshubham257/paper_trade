...
function assertMinimumIacCliVersion() {
  ...
  const executable = process2__default.default.env._ || "railway";
  let output = "";
  try {
    output = child_process.execFileSync(executable, ["--version"], {
      encoding: "utf8",
...
function isNativeCliEvaluation() {
  const evalFlag = process2__default.default.execArgv.indexOf("-e");
  const source = evalFlag >= 0 ? process2__default.default.execArgv[evalFlag + 1] : void 0;
  return process2__default.default.execArgv.includes("--experimental-strip-types") && process2__default.default.execArgv.includes("--input-type=module") && source?.includes("const partial = mod.partial") === true && source.includes("process.stdout.write(JSON.stringify({ partial, project: graph }))");
}
...
function signalBucketRatio(salt, subject) {
  const digest = crypto.createHash("sha256").update(`${salt}:${subject}`).digest();
...
var SIGNALS_WITH_OWNER_QUERY = graphql.parse(`
...
`);
var SIGNALS_INFERRED_QUERY = graphql.parse(`
...
exports.ExecInterruptedError = ExecInterruptedError;
exports.FlagsScopeError = FlagsScopeError;
exports.RAILWAY_GRAPH_VERSION = RAILWAY_GRAPH_VERSION;
exports.RAILWAY_PROJECT_ID_ENV = RAILWAY_PROJECT_ID_ENV;
...
exports.STALE_ENVIRONMENT_BASE_CODE = STALE_ENVIRONMENT_BASE_CODE;
...
exports.StaleEnvironmentError = StaleEnvironmentError;
...
exports.empty = empty;
exports.evaluateFlagRuleset = evaluateFlagRuleset;
exports.evaluateFlagRulesetSync = evaluateFlagRulesetSync;
exports.flags = flags;
exports.fn = fn;
...
exports.mongo = mongo;
exports.mysql = mysql;
exports.normalizeEvaluationContext = normalizeEvaluationContext;
...
exports.signalBucketRatio = signalBucketRatio;
