...
function assertMinimumIacCliVersion() {
  ...
  const executable = process__default.default.env._ || "railway";
  let output = "";
  try {
...
function isNativeCliEvaluation() {
  const evalFlag = process__default.default.execArgv.indexOf("-e");
  const source = evalFlag >= 0 ? process__default.default.execArgv[evalFlag + 1] : void 0;
  return process__default.default.execArgv.includes("--experimental-strip-types") && process__default.default.execArgv.includes("--input-type=module") && source?.includes("const partial = mod.partial") === true && source.includes("process.stdout.write(JSON.stringify({ partial, project: graph }))");
}
