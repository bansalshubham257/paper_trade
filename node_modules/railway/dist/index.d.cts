export { B as BucketConfig, a as BucketNode, b as BucketRegion, C as CompileResult, D as DatabaseConfig, c as DatabaseNode, E as EnvironmentConfig, G as GraphCompileOptions, d as GraphIndex, P as ProjectDefinition, e as ProjectNode, R as RAILWAY_GRAPH_VERSION, f as RailwayGraph, g as RailwayProgram, h as RailwayResourceName, i as RailwayServiceName, j as ResourceAddress, k as ResourceNode, S as ServiceConfigInput, l as ServiceNode, V as VariableValue, m as bucket, n as createRailwayContext, o as database, p as define, q as defineRailway, r as empty, s as fn, t as github, u as group, v as iac, w as image, x as indexGraph, y as mongo, z as mysql, A as postgres, F as project, H as redis, I as ref, J as resourceAddress, K as service, L as template, M as validateGraph, N as volume } from './index-C3uk0ruc.cjs';
...
interface NormalizedRailwayClientConfig {
    token: string;
    authType: RailwayAuthType;
    endpoint: string;
    fetch: typeof fetch;
...
declare class ExecInterruptedError extends RailwayConnectionError {
...
type RadarOperator = "eq" | "neq" | "gt" | "lt" | "gte" | "lte" | "in_list" | "not_in_list" | "contains" | "not_contains" | "matches";
interface RadarClause {
    attr: string;
    op: RadarOperator;
...
} | RadarClause | SignalBucketCompare;
type SignalType = "bool" | "string" | "number" | "json";
type SignalSource = {
    type: "literal";
    value: unknown;
} | {
...
type SignalResolutionOutcome = "default" | "agreed" | "disagreement" | "no_match";
interface SignalResolutionRuleTrace {
    ruleId: string;
    matched: boolean;
    source: SignalSource;
    value: unknown | null;
}
interface SignalResolutionTrace {
    signalName: string;
    default: unknown;
...
type Reason = "TARGETING_MATCH" | "SPLIT" | "NO_MATCH" | "CONFLICT" | "NOT_FOUND" | "TYPE_MISMATCH" | "STALE" | "NO_KEY";
interface TraceStep {
    ruleId: string;
    matched: boolean;
    value: unknown | null;
...
declare class FlagsRegistry {
    #private;
    readonly scopeOwner: string | undefined;
    synced: boolean;
    stale: boolean;
    constructor(scopeOwner: string | undefined);
    has(name: string): boolean;
    list(): string[];
    load(config: NormalizedRailwayClientConfig): Promise<void>;
    markStale(): void;
    evaluate<T>(name: string, expected: SignalType | null, ctx: Context | undefined, fallback: T, zero: T, onConflict: ConflictReporter): Evaluation<T>;
}
declare class FlagsReadSurface {
    private readonly registry;
    private readonly assertReady;
    private readonly onConflict;
    private readonly getConfig;
    constructor(registry: FlagsRegistry, assertReady: () => void, onConflict: ConflictReporter, getConfig: () => NormalizedRailwayClientConfig);
    ...
    evaluateJson<T = Record<string, never>>(name: string, ctx?: Context, fallback?: T): Evaluation<T>;
    has(name: string): boolean;
    list(): string[];
    refresh(): Promise<void>;
}
declare class FlagsModule {
    #private;
    get ready(): Promise<void>;
    get synced(): boolean;
    init(options?: FlagsInitOptions): Promise<void>;
...
declare const RAILWAY_PROJECT_ID_ENV = "RAILWAY_PROJECT_ID";
declare class FlagsScopeError extends RailwayError {
    constructor();
}
declare function serializeScope(scope: FlagsScope): string;

/** Normalize the public flat `Context` into the internal resolver shape. */
declare function normalizeEvaluationContext(raw: unknown): FlagEvaluationContext;

declare function evaluateFlagRuleset(ruleset: SignalRuleset, context?: FlagEvaluationContext | Context): Promise<SignalResolveResult & {
    reason: Reason;
}>;
declare function evaluateFlagRulesetSync(ruleset: SignalRuleset, context?: FlagEvaluationContext | Context): SignalResolveResult & {
    reason: Reason;
};

/** Deterministic percentage bucket in [0, 1) for rollout expressions. */
declare function signalBucketRatio(salt: string, subject: string): number;

export { type CheckpointOptions, type ConnectOptions, type Context, type CreateOptions, DEFAULT_RAILWAY_GRAPHQL_ENDPOINT, type Evaluation, ExecHandle, ExecInterruptedError, type ExecOptions, type ExecReattachTarget, type ExecResult, type ExecSignal, type ExecTarget, type FileReadFormat, type FileReadOptions, type FileWriteData, type FileWriteOptions, type FlagEvaluationContext, type FlagsInitOptions, type FlagsScope, FlagsScopeError, type ForkOptions, type ListOptions, RAILWAY_PROJECT_ID_ENV, RailwayAuthError, type RailwayClientConfig, RailwayConnectionError, RailwayError, RailwayGraphQLError, type RailwayGraphQLErrorItem, type Reason, STALE_ENVIRONMENT_BASE_CODE, Sandbox, type SandboxCheckpointInfo, SandboxFailedError, type SandboxFileEntry, SandboxFileNotFoundError, SandboxFiles, SandboxFilesError, type SandboxInfo, type SandboxNetworkIsolation, SandboxNotFoundError, type SandboxStatus, type SandboxTemplate, SandboxTemplateBuildError, SandboxTimeoutError, type SignalExpression, type SignalResolutionTrace, type SignalRule, type SignalRuleset, type SignalType, StaleEnvironmentError, type TemplateBuildOptions, type TraceStep, evaluateFlagRuleset, evaluateFlagRulesetSync, flags, normalizeEvaluationContext, serializeScope, signalBucketRatio };
